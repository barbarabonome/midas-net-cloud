...
// Register Health Check services para inje��o de depend�ncia
...
// Middleware para logging de requisi��es com correla��o
app.Use(async (context, next) =>
{
  var correlationId = context.Request.Headers.ContainsKey("X-Correlation-ID")
    ...
    {
    Log.Information("Requisi��o iniciada: {Method} {Path}", context.Request.Method, context.Request.Path);
  await next(context);
        Log.Information("Requisi��o finalizada com status: {StatusCode}", context.Response.StatusCode);
 }
});
...
Console.WriteLine("Aplicação rodando na porta configurada.");
